...
const Message = ({ role, content, timestamp }: any) => {
  ...
  return (
    <div className={cn("flex w-full mb-8 group animate-in fade-in slide-in-from-bottom-2", isUser ? "justify-end" : "justify-start")}>
      <div className={cn("flex gap-4 max-w-[85%] md:max-w-[75%]", isUser ? "flex-row-reverse" : "flex-row")}>
        ...
        <div className={cn(
          ...
          isUser ? "bg-slate-900 border-slate-800 text-white" : "bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 text-indigo-600"
        )}>
        ...
        <div className={cn("flex flex-col gap-2", isUser ? "items-end" : "items-start")}>
          ...
          <div className="flex items-center gap-2 px-2 opacity-50">
            ...
            <div className={cn("h-1 w-1 rounded-full", isUser ? "bg-indigo-400" : "bg-emerald-500 animate-pulse")} />
          </div>
        </div>
      </div>
    </div>
          ...
          >
            <div className="flex flex-col gap-1 overflow-hidden">
              ...
              <div className="flex items-center gap-1.5 opacity-60">
                ...
                <span className="text-[8px] font-bold uppercase tracking-widest">Active Link</span>
              </div>
            </div>
  ...
  return (
    <div className="flex h-full w-full bg-[#FDFDFD] dark:bg-[#0B0E14] overflow-hidden">
      ...
      <aside className={cn(
        ...
        isSidebarOpen ? "w-80 translate-x-0" : "w-0 -translate-x-full lg:translate-x-0 lg:w-0"
      )}>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar min-w-[320px]">
          <ConversationList conversations={conversations} currentId={currentId} onSelect={(id:any) => {
        ...
        <div className="p-6 md:p-10 bg-linear-to-t from-[#FDFDFD] dark:from-[#0B0E14] via-[#FDFDFD]/90 dark:via-[#0B0E14]/90 to-transparent">
            ...
            <div className="group relative flex items-center transition-all duration-500">
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    <span className="hidden md:inline text-[10px] font-black uppercase tracking-widest italic">Execute</span>
          ...
          <div className="mt-4 flex justify-center gap-8 opacity-40">
            <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.2em] text-slate-500">
               <div className="h-1 w-1 bg-emerald-500 rounded-full animate-pulse" /> Neural Link: Online
            </div>
            <div className="flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.2em] text-slate-500">
               <div className="h-1 w-1 bg-indigo-500 rounded-full" /> Encryption: AES-256
            </div>
          </div>
        </div>
      </div>
